// =======================
// Loja Escolar - Frontend
// Busca, ordenação persistida, carrinho em localStorage,
// cupom ALUNO10, paginação, export CSV/JSON, drawer acessível
// =======================
...
function loadCarrinho() {
  try { return JSON.parse(localStorage.getItem(LS_KEYS.CARRINHO)) ?? []; }
  catch { return []; }
}
function saveCarrinho(c) {
  localStorage.setItem(LS_KEYS.CARRINHO, JSON.stringify(c));
  atualizarBadge();
}
function loadOrdenacao() {
  return localStorage.getItem(LS_KEYS.ORDENACAO) || 'nome-asc';
}
function saveOrdenacao(v) {
  localStorage.setItem(LS_KEYS.ORDENACAO, v);
}
function loadCupom() {
  return localStorage.getItem(LS_KEYS.CUPOM) || '';
}
function saveCupom(v) {
  localStorage.setItem(LS_KEYS.CUPOM, v);
}
...
function filtrarOrdenar() {
  ...
  let base = produtos.filter(p => normalizar(p.nome).includes(termo));
...
function renderProdutos() {
  ...
  visiveis.forEach(prod => {
    ...
    card.innerHTML = `
      <img src="https://placehold.co/200x200?text=${encodeURIComponent(prod.nome)}" alt="Imagem ilustrativa de ${prod.nome}" />
      <strong>${prod.nome}</strong>
      <div>${fmtBRL(prod.preco)}</div>
      <div style="font-size:.9rem;color:#94a3b8;">Estoque: ${prod.estoque}</div>
    ...
    carrinho.forEach(item => {
      ...
      row.innerHTML = `
        <div>
          ...
          <div style="font-size:.9rem;color:#94a3b8;">${fmtBRL(item.preco)} — Em estoque: ${max}</div>
...
function adicionarCarrinho(id, qtd = 1) {
  ...
  if (disponivel <= 0) {
    anunciar(`Sem estoque disponível para ${p.nome}.`);
    return;
  }
...
elFinalizar?.addEventListener('click', () => {
  ...
  for (const item of carrinho) {
    ...
    if (!p || item.qtd > p.estoque) {
      alert(`Estoque insuficiente para ${item?.nome ?? 'produto'}.`);
      return;
    }
  }
  for (const item of carrinho) {
...
elBtnLimpar.addEventListener('click', () => {
  elBusca.value = '';
  elOrdenar.value = 'nome-asc';
  saveOrdenacao('nome-asc');
  pagina = 1;
  renderProdutos();
});
